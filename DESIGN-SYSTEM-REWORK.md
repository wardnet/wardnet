...
| CreateTunnelInline       | [x]    | inline form + WireGuard config paste — already conformant: imports only Forge `Card`/`Button`/`Tabs` primitives + `ManualTunnelTab` / `ProviderTunnelTab` siblings; no shadcn / hex / inline-styles / non-token Tailwind palette refs. Public API (`onClose`, `embedded?`) preserved (slice T4-β). |
| DashboardLogWidget       | [x]    | `.logs` via `LogViewer` (T3-γ) — header reshaped to `Card`/`CardHeader`/`CardTitle`/`CardAction` with controls (level Select, Pause/Resume, Clear, Download) inside `CardAction`; dropped per-control `h-8`/`text-xs`/`mr-1 size-3` overrides — Forge `Button size="sm"` and `SelectTrigger` own density. Lucide icons renamed `*Icon` per UpdateCard precedent. Slice T4-β on 2026-05-10. |
...
| DeviceNetworkCard        | [x]    | edit-mode card protocol per DhcpConfigCard / DeviceSettingsCard precedent — already on Forge `Card`/`CardHeader`/`CardAction`/`CardContent`/`CardFooter` + `Button`/`Field` primitives with `Ipv4Input`, `StatusBadge`, and `ApiErrorAlert`; edit-mode replaces the prior raw `<button>Remove reservation</button>` inline link with a Forge `<Button variant="destructive" size="sm">` anchored left in the `CardFooter` via `mr-auto` while Cancel (`ml-auto`) and Save stay right-aligned. Hook-coupled (`useDhcpReservations` / `useCreateReservation` / `useDeleteReservation`); no shadcn / hex / inline-styles. Public API (`device`) preserved (slice T4-α on 2026-05-10). |
| DeviceSettingsCard       | [x]    | edit-mode card protocol — Card/CardHeader/CardAction (Edit) → CardContent/CardFooter (Cancel + Save) on `editing`; Forge `Button`/`Card`/`Field`/`Input`/`Select`/`Toggle` primitives, `text-ink-3` token; no shadcn / hex / inline-styles. This file is the reference precedent cited by `DhcpConfigCard` — verified conformant on 2026-05-10 (slice T4-α), no code changes required. |
| DnsFilterSettingsCard    | [x]    | settings-page card for the global DNS filter (kill-switch + default profiles) — already conformant on Forge `Card`/`Field`/`Toggle` primitives. Diverges from edit-mode precedent on purpose: kill switch and default-profile assignments are instant-apply (mutation fires from `onCheckedChange` / `ProfileToggleList.onChange`, with `disabled={update.isPending}` covering the in-flight window) so an Edit/Save cycle would block the emergency stop. Verified on 2026-05-10 (slice T4-α), no code changes required. |
| DnsStatsSection          | [x]    | chart + stat tiles — uses ChartContainer (slice 1a) with `var(--chart-1)`/`var(--chart-2)` palette; SVG `stroke` reads on `<Line>`/`<Brush>` switched to `var(--color-total)` / `var(--color-blocked)` per the cssVars bridge precedent (TunnelThroughputChart slice 1a); StatTile primitive (slice 2a) consumed via DashboardStatCard. **Donut DEFERRED** — no consumer; row's "donut" mention was speculative. Slice T4-β on 2026-05-10. |
| EditDhcpConfigSheet      | [-]    | subsumed by DhcpConfigCard edit-mode (sheet dropped) |
| ManualTunnelTab          | [x]    | tabbed sub-form — already conformant: imports `Button`/`Field`/`Input`/`Textarea` from forge-web, `ApiErrorAlert` compound, `useCreateTunnel` hook; no shadcn / hex / inline-styles / non-token Tailwind palette refs. Public API (`onSuccess`) preserved (slice T4-β). |
| PowerCard                | [x]    | danger-toned actions outside card per skill §detail — Reboot + Restart-daemon stay inside Forge `Card`; Safe Shutdown moved to a sibling `bg-danger-soft`/`text-danger-soft-ink`/`border-danger-soft` row OUTSIDE the card with `Button variant="destructive"`. Confirm dialogs use Forge `AlertModal`. Public API (`onReboot`, `onShutdown`, `onRestartDaemon`, `busy`) preserved (slice T4-β). |
| ProviderTunnelTab        | [x]    | tabbed sub-form — already conformant on Forge `Button`/`Field`/`Input`/`Select` + `CountryCombobox`/`ApiErrorAlert` compounds; `text-accent-ink`/`text-warn-soft-ink`/`text-danger`/`text-ink-3`/`bg-sunken` resolve to Forge `@theme` tokens. Public API (`onSuccess`) preserved (slice T4-β). |
...
| TunnelDevicesTable       | [x]    | `.tbl` + `.host` — ported off raw Table primitives onto `DataTable` + `HostCell` per the T3-α DeviceTable shape; `buildColumns()` + `meta.className` for responsive hiding; empty-state moved to `DataTable.emptyMessage` so it renders inside the `.tbl` shell. Public API (`tunnelId`) preserved (slice T4-β). |
| TunnelThroughputChart    | [x]    | line chart §10 (Download = `--accent`, Upload = `--info`) — Recharts CartesianGrid stripped of inline `strokeDasharray` (Forge `.chart` CSS owns 2 4 horizontal hairlines + hides vertical grid); XAxis `tick fontSize` removed (Forge sets 12px / `--ink-3`); Line strokeWidth 2 → 1.75 (lands inside §10 1.6-1.8 band); custom YAxisTick simplified to bare `<text>` (Forge mono / ink-3 inherits via `.recharts-yAxis .recharts-cartesian-axis-tick text`). chartConfig already on `var(--chart-1)` / `var(--chart-2)` from slice 1a. Public API (`tunnelId`, `range`, `onRangeChange`) preserved (slice T4-β). |
...
| DeviceDetail          | [x]    | detail-screens.jsx — composes DetailPageHeader + DeviceIdentityCard + DeviceSettingsCard + DeviceDnsFilterCard + DeviceNetworkCard (all already ported with edit-mode protocol). Page wrapper switched to Forge `col gap-20`; not-found branch uses `.h-title`/`.h-sub` (Forge tokens, no shadcn). Slice T6-β on 2026-05-10. |
...
| TunnelDetail          | [x]    | detail-screens.jsx — composes DetailPageHeader + TunnelThroughputChart + TunnelDevicesTable (all ported). Bespoke MetadataRow replaced by Forge `Field` (read-only) per DeviceIdentityCard precedent; mono values via `.mono` utility; loading/error use `.h-title`/`.h-sub`. Slice T6-β on 2026-05-10. |
...
| Dns                   | [x]    | DNS resolver config page (toggle, cache, retention, upstreams) — page wrapper switched to Forge `col gap-20`; redundant `flex-row` dropped from CardHeaders; upstream-empty fallback now `.empty` Forge class. NOTE: this is the resolver config page; query-stream lives in DnsLogs (separate row 263). Slice T6-β on 2026-05-10. |
...
| Setup (+ setup/*)     | [x]    | multi-step wizard — Setup.tsx + WizardStepper + 7 step components ported in one slice. Dropped inner `rounded-2xl bg-white/95 ... shadow-2xl backdrop-blur-sm` double-card wrapper (AuthLayout's Card is now sole host). WizardStepper: string-concat className → clsx, `border-ink-3/30` → `border-line`. Step headings → `.h-title`/`.h-sub`; `bg-[oklch(...)]` literals + `h-12` overrides removed (Forge `btn--primary` defaults). Step 2 IP-not-pinned warn callout uses `bg-warn-soft`+`text-warn-soft-ink` (inline, not Banner — full-width strip not needed at this width). Slice T6-δ on 2026-05-10. |
| Login                 | [x]    | AuthLayout, single card — dropped inner `rounded-2xl bg-white/95 ... shadow-2xl backdrop-blur-sm dark:bg-card/95` double-card wrapper so AuthLayout's Forge Card is the sole host. Submit button `bg-[oklch(0.22_0.12_275)] dark:bg-accent` literal + `h-12` overrides + Input `h-12` overrides all dropped (Forge `Button`/`Input` primitive sizing now wins). Slice T6-δ on 2026-05-10. |
| NotFound              | [x]    | `.empty` full-page — replaced bespoke `flex flex-col items-center justify-center gap-4 py-20` wrapper with full-page Forge `.empty` block; 404 numeral + "Page not found" heading both use `.h-title`; explainer uses `.h-sub max-w-md`; home button is `Button variant="outline"`. Slice T6-δ on 2026-05-10. |
