...
│   └── wardnet-go/                  # wardnet.network/go — Go SDK; VERSION here drives the published module version
│       ├── internal/rest/           # Generated by oapi-codegen from docs/openapi.json; `make check-go-openapi` fails on drift
│       └── *.go                     # Hand-crafted public API: Client + per-area services, domain types, functional options, typed APIError
├── wctl/                            # github.com/wardnet/wardnet/source/wctl — Cobra CLI on the Go SDK (replace => ../sdk/wardnet-go)
...
│   └── scripts/install.sh           # Downloads a release binary + verifies it against checksums.txt
