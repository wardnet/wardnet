...
- **Typography** — render text through the `<Text>` / `<Heading level>` primitive (from `@wardnet/ui`, re-exported by `@wardnet/web`); see `docs/adr/0012-typography-scale-and-roles.md`.
  ...
  - **Enforced in admin-site, user-app, and admin-app.** Each has a `tests/typography-conventions.test.ts` that scans its own `src/**/*.tsx` and fails CI if a raw `text-*`/`font-*` size or weight utility reappears. Vendored shadcn primitives under admin-site's `components/core/ui` are exempt (the PWAs have no vendored tree); a justified one-off opts out with a `ds-typography-allow:` comment on or just above the line.
  - **Primitives that swallow `asChild`.** `DrawerTitle` renders its own `<h3>` and drops `asChild`, so it can't *become* a `<Text>`. Nest the primitive instead — `<DrawerTitle className="<layout+colour>"><Text as="span" size=… weight=…>…</Text></DrawerTitle>`. Same shape for a bare `<button>` / `<a>` / `<Link>` carrying label typography: keep layout and colour on the interactive element and wrap only the label, so interactive semantics and the accessible name are unchanged.
