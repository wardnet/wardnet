 ...
 * SVG path set:
 *
 * - Chrome (size < 40px) — used in the Navbar. The shield is filled solid;
 *   only the signal arcs and ping dot show through. Reads as a compact accent
 *   chip at 24-28px.
 * - Marketing (size >= 40px) — used in Hero / Footer / error pages. The shield
 *   gets a hairline `--ink` outline and the signal arcs gain a second, wider
 *   ring so the "signal" half of the mark is legible at 60px+.
 *
 * Colors come from Forge tokens (`--accent`, `--ink`, `--side-bg`) so the
 * mark retints automatically with the active theme. Shared shape with
 * `admin-app` Logo — keep the two SVGs in sync if either is touched.
 */
