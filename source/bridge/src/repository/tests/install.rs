...
async fn update_ip() {
    ...
    repo.update_ip("id-4", "203.0.113.1", "cf-record-abc", Utc::now())
        .await
        .unwrap();
...
async fn registration_rate_limit_log() {
    ...
    repo.log_registration("1.2.3.4", Utc::now()).await.unwrap();
    repo.log_registration("1.2.3.4", Utc::now()).await.unwrap();
    repo.log_registration("5.6.7.8", Utc::now()).await.unwrap();
