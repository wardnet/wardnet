...
/// Public-facing `CalVer` (`YYYY.MM.DD`) read at compile time from
/// the workspace-root `CALVER` file. Stable across dev rebuilds —
/// use this for anything user-visible (web UI, `OpenAPI`
/// `info.version`, /api/info `release_version`) or for the
/// auto-update version comparison against the published manifest.
