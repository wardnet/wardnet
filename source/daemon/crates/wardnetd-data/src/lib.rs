...
use repository::{
    AdminRepository, AnomalyRepository, ApiKeyRepository, DeviceIdentificationRepository,
    ...
    RuleRequestRepository, SessionRepository, SqliteAdminRepository, SqliteAnomalyRepository,
...
pub trait RepositoryFactory: Send + Sync {
    ...
    fn device_identification(&self) -> Arc<dyn DeviceIdentificationRepository>;
    ...
    fn private_dns_grants(&self) -> Arc<dyn PrivateDnsGrantRepository>;
...
impl RepositoryFactory for SqliteRepositoryFactory {
    ...
    fn device_identification(&self) -> Arc<dyn DeviceIdentificationRepository> {
        Arc::new(SqliteDeviceIdentificationRepository::new_pools(
            self.pools.clone(),
        ))
    }
    ...
    fn private_dns_grants(&self) -> Arc<dyn PrivateDnsGrantRepository> {
        Arc::new(SqlitePrivateDnsGrantRepository::new_pools(
            self.pools.clone(),
        ))
    }
