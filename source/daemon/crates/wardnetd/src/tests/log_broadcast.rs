//! Tests for [`LogBroadcaster`], [`BroadcastLayer`], and [`LogEntry`].
//!
//! Since `LogBroadcaster::send` is private, all end-to-end send/receive tests
//! exercise the full pipeline through `BroadcastLayer` + tracing.

use crate::log_broadcast::{BroadcastLayer, LogBroadcaster, LogEntry};
use std::collections::BTreeMap;
use tracing_subscriber::layer::SubscriberExt;

// ---------------------------------------------------------------------------
// LogBroadcaster unit tests (public API only)
// ---------------------------------------------------------------------------

#[test]
fn new_creates_broadcaster_without_panic() {
    let _broadcaster = LogBroadcaster::new(16);
}
...
fn clone_shares_underlying_channel() {
    ...
    // Both should be able to subscribe without issue.
    let _rx1 = broadcaster.subscribe();
    let _rx2 = clone.subscribe();
}
...
// ---------------------------------------------------------------------------
// BroadcastLayer integration tests (exercises send + receive)
// ---------------------------------------------------------------------------

#[tokio::test]
async fn layer_captures_info_event() {
    let broadcaster = LogBroadcaster::new(64);
    let mut rx = broadcaster.subscribe();

    let layer = BroadcastLayer::new(broadcaster);
