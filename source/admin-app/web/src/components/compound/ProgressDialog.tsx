...
export function ProgressDialog({
  ...
  const elapsed = useElapsedSeconds(open && phase === "in-progress" ? startedAt : null);
      ...
      <AlertModalContent>
        <AlertModalHeader>
          <AlertModalTitle className="flex items-center gap-2">
            {phase === "in-progress" && <Loader2Icon className="h-5 w-5 animate-spin text-ink-3" />}
            {phase === "success" && successIcon}
            {phase === "failed" && <AlertTriangleIcon className="h-5 w-5 text-danger" />}
