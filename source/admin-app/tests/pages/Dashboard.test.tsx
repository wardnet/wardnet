...
describe("Dashboard", () => {
  beforeEach(() => {
    ...
    h.useDaemonStatus.mockReturnValue({ data: { reachable: true, uptimeSeconds: 60 } });
    h.useDashboardDnsStats.mockReturnValue({
      data: { total: 10, blocked: 1, blockedPercent: 10, totalSeries: [], blockedSeries: [] },
      isLoading: false,
    });
  });
  ...
  it("renders every dashboard card", () => {
    ...
    expect(screen.getByTestId("dashboard-dns-queries-card")).toBeInTheDocument();
  ...
  it("falls back to defaults when queries have no data", () => {
    ...
    h.useDashboardDnsStats.mockReturnValue({ data: undefined, isLoading: true });
    renderWithProviders(<Dashboard />);
