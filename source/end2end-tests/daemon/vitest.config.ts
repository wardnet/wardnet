...
export default defineConfig({
  test: {
    ...
    // toggle, race on lease state). fileParallelism:false (vitest 4
    // replacement for poolOptions.forks.singleFork) keeps every file in
    // one process and `isolate: false` shares the module graph across
