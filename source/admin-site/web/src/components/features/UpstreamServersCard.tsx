...
export function UpstreamServersCard({ servers, isSaving, onUpdate }: UpstreamServersCardProps) {
  ...
  const columns = useMemo<ColumnDef<UpstreamDns & { __index: number }>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => <Pill variant="ghost">{row.original.protocol.toUpperCase()}</Pill>,
      },
    ],
    [],
  );

  const rows = useMemo(() => servers.map((s, i) => ({ ...s, __index: i })), [servers]);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardAction>
          {!adding && (
            <Button variant="outline" onClick={() => setAdding(true)} disabled={isSaving}>
              Add server
            </Button>
          )}
        </CardAction>
      ...
      <CardContent className="flex flex-col gap-4">
        {adding && (
          <AddServerForm onCancel={() => setAdding(false)} onSubmit={add} isSaving={isSaving} />
        )}
        ...
        {servers.length === 0 ? (
          ...
          <DataTable
            ...
            rowActions={(row) => (
              <>
                <RowAction onSelect={() => move(row.__index, -1)} icon={<ArrowUp aria-hidden />}>
                  Move up
                </RowAction>
                <RowAction onSelect={() => move(row.__index, 1)} icon={<ArrowDown aria-hidden />}>
                  Move down
                </RowAction>
      ...
      <Form values={{ name, address, port }} onSubmit={handleSave}>
        <CardContent className="flex flex-col gap-5">
          <div className="flex gap-3">
            <Field label="Name" htmlFor="ups-name" name="name" className="flex-1">
              <Input
                id="ups-name"
                value={name}
            ...
            <Validator name="name" rule="required" message="Name is required." />

            <Field label="Protocol" htmlFor="ups-proto" className="flex-1">
              <Select value={protocol} onValueChange={(v) => setProtocol(v as DnsProtocol)}>
          ...
          <div className="flex gap-3">
            <Field label="Address" htmlFor="ups-addr" name="address" className="flex-1">
              <Input
                id="ups-addr"
                value={address}
            ...
            <Validator name="address" rule="required" message="Address is required." />

            <Field label="Port (optional)" htmlFor="ups-port" name="port" className="flex-1">
              <Input
                id="ups-port"
                type="number"
                value={port}
