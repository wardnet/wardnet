...
import { DataTable, RowAction, type DataTableGroup } from "@/components/core/ui/data-table";
...
import type { Device, DhcpLease, DhcpLeaseStatus, DhcpReservation } from "@wardnet/js";
...
function leaseTypeBadge(status: DhcpLeaseStatus): ReactNode {
  ...
  return (
    <div className="flex flex-col items-start gap-1 md:flex-row md:items-center md:gap-1.5">
      ...
      <StatusBadge tone={status === "active" ? "success" : "neutral"}>{label}</StatusBadge>
    </div>
  );
}
...
function buildColumns(deviceIndex: Map<string, Device>): ColumnDef<DhcpEntry>[] {
  return [
    {
      id: "host",
      header: "Host",
      cell: ({ row }) => {
        ...
        const primary =
          device?.name ?? entry.description ?? entry.hostname ?? device?.hostname ?? entry.mac;
        ...
        const icon = device ? <DeviceIcon type={device.device_type} /> : fallbackIcon;
        return (
          <HostCell
            primary={primary}
            secondary={secondary}
    ...
    <DataTable
      ...
      rowActions={(entry) => {
        if (entry.kind === "reservation" && entry.reservation) {
          return (
            <RowAction onSelect={() => onDeleteReservation(entry.reservation!.id)} destructive>
              Delete
            </RowAction>
          );
        }
        if (entry.kind === "lease" && entry.lease && entry.leaseStatus === "active") {
          return (
            <>
              <RowAction onSelect={() => onMakeStatic(entry.lease!)}>Make static</RowAction>
              <RowAction onSelect={() => onRevokeLease(entry.lease!.id)} destructive>
                Revoke
              </RowAction>
            </>
          );
