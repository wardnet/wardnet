...
function ProfileCountBadges({ profileId }: { profileId: string }) {
  ...
  const blockedCount = (blData?.blocklists ?? []).reduce((sum, b) => sum + b.entry_count, 0);
  ...
  return (
    <div className="flex flex-wrap justify-end gap-2">
      {blockedCount > 0 && (
        <Pill variant="ghost">{blockedCount.toLocaleString()} blocked domains</Pill>
      )}
      {allowedCount > 0 && (
        <Pill variant="ghost">{allowedCount.toLocaleString()} allowed domains</Pill>
      )}
      {customCount > 0 && <Pill variant="ghost">{customCount.toLocaleString()} custom rules</Pill>}
    </div>
  );
}
  ...
  return (
    <div className="col gap-20">
      <PageHeader
        ...
        actions={hasProfiles ? <Button onClick={addProfile}>Add profile</Button> : undefined}
      />
