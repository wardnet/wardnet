...
.PHONY: all init build build-daemon build-web build-pi \
        check check-web check-daemon fmt clippy test \
...
help:
	...
	@echo ""
	@echo "  check          Run all checks (web + daemon)"
	@echo "  check-web      Typecheck + lint + format check for web UI"
