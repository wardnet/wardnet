...
**Premium tier** — Paid. Grants the two wardnet-operated, cost-bearing capabilities: the **DDNS service** (a managed `<slug>.my.wardnet.services` via the **wardnet DnsProvider**) and the **Tunneler** (private DNS while roaming) — plus the mobile app surfaces (the user PWA and admin mobile app), which are Premium-only regardless of whether the box brings its own domain. Free/BYO-domain installs administer entirely through the desktop admin website and `/api/*`, which stay reachable on every tier.
