...
### Enabling remote access no longer fails with "upstream unavailable"

Registering for remote access probes each cloud region's health to pick the closest one — and the probe was aimed at a health endpoint the cloud's health tier no longer serves for that hostname, so every region looked unreachable and registration failed with "upstream unavailable" before ever contacting the cloud. The probe now targets the region's DNS service health endpoint directly — the exact service registration is about to talk to. Closes [#901](https://github.com/wardnet/wardnet/pull/901).
