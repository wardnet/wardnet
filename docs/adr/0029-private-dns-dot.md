...
- **The naming fix is now load-bearing on docs.** "Tunneller" (two Ls, matching the `/tunneller/…` wire path and the `cloud/tunneller*.rs` modules) is the relay; "Private DNS" is the feature. CONTEXT.md and this ADR use the two-L spelling exclusively; one-L `Tunneler` deliberately survives in the `TunnelerClient` / `TunnelerConnector` / `TunnelerRunner` identifiers and in the prose of ADR-0010 and ADR-0011. Both were left alone on purpose: renaming the identifiers would churn the cloud clients for a docs change, and **an accepted ADR is a record of a decision as it was made, not a live document** — ADR-0010's "the Tunneler (private DNS while roaming)" is precisely the conflation this ADR supersedes, and editing it would erase the evidence that the conflation existed.
