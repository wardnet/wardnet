...
- **Upstream selection is per-device, not per-IP, for the same reason (#923).** The per-device DNS routing snapshot is keyed by LAN source IP and reflects *applied* kernel state, both of which a roaming device lacks — v1 therefore sent roaming queries to the default upstream. #923 adds a second, `device_id`-keyed snapshot built by the routing service from the **persisted** routing rules (with `Default` resolved through the global policy, gated on `override_default_dns` and the tunnel not being `Down`), and the pipeline resolves a device-authenticated client's upstream through it: token → device → tunnel binding → `forward_via_tunnel`, so a device routed through the UK tunnel gets UK DNS answers on the go too. A `Down` tunnel drops the device to the default upstream — the same soft fallback the LAN path takes — and a transiently unbuildable forwarder fails **closed** (`SERVFAIL`), never leaking to the default upstream mid-binding. Zone egress stays honored by construction: a zone unbind rewrites the persisted rule and fires `RoutingRuleChanged`, which rebuilds the snapshot. When per-device kill-switch mode (#235) lands, its `block` mode belongs in the snapshot's down-tunnel gate: a block-mode device must keep failing rather than fall back.
